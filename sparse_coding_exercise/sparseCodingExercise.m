...
if DEBUG
	disp('display patches over.press to continue!');
	pause;
end
if ~SKIP
%%======================================================================
...
if DEBUG
	disp('check weightMatrix over.press to continue!');
	pause;
end
...
if DEBUG
	disp('check featureMatrix over.press to continue!');
	pause;
end
...
assert(diff < 1e-8, 'Feature difference too large. Check your feature cost function. ');
if DEBUG
	disp('check topographic featureMatrix over.press to continue!');
	pause;
end
end
%%======================================================================
...
for iteration = 1:200                      
    ...
    %   optimization.
    [n m] = size(batchPatches);
	weightMatrix = (batchPatches*featureMatrix')/(m*gamma*eye(numFeatures)+featureMatrix*featureMatrix');
	
	if ~WEIGHT_CHECK_SKIP
