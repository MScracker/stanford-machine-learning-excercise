...
for i = 1:MAX_ITR
    ...
    (lambda(j)/(2*m))*norm(theta([2:end]))^2;
    
    % Calculate gradient and hessian.
    G = (lambda(j)/m).*theta; G(1) = 0; % extra term for gradient
    L = (lambda(j)/m).*eye(n); L(1) = 0;% extra term for Hessian
    grad = ((1/m).*x' * (h-y)) + G;
...
%hold off
 ...
 %Uncomment to plot J
 plot(0:MAX_ITR-1, J, char(plotStyle(j)), 'MarkerFaceColor', 'r', 'MarkerSize', 8)
 hold on;
